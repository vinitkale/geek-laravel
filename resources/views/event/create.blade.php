        ...
        <div class="row">
            <div class="col-md-12 col-xs-12">
                ...
                <div class="x_panel">
                    ...
                    <form id="event-form" action="{{URL::to('event')}}" method="post" enctype="multipart/form-data" >
                        <div class="col-md-6 col-xs-12">
                            <div class="x_content">

                                <!-- start form for validation -->
                                <label for="fullname">Event Title* :</label>
                                <input id="event_title" class="form-control" value="{{ old('event_title') }}" name="event_title" type="text"><ul class="parsley-errors-list">{{ $errors->event->first('event_title') }}</ul>

                                <label for="fullname">Start Date* :</label>
                                <input id="start-date" class="form-control" name="start_date" type="text" value="<?php echo (old('start_date') != "") ? old('start_date') : date('d-m-Y', time()); ?>"><ul class="parsley-errors-list">{{ $errors->event->first('start_date') }}</ul>

                                <label for="fullname">Start Time* :</label>
                                <input class="form-control" name="start_time" id="event-start-time" type="text" value="<?php echo (old('start_time') != '') ? old('start_time') : date("h:i:s", time()); ?>"><ul class="parsley-errors-list">{{ $errors->event->first('start_time') }}</ul>

                                <label for="fullname">End Date* :</label>
                                <input id="end-date" class="form-control" name="end_date" type="text" value="<?php echo (old('end_date') != "") ? old('end_date') : date('d-m-Y', time()); ?>"><ul class="parsley-errors-list">{{ $errors->event->first('end_date') }}</ul>



                                <label for="fullname">End Time* :</label>
                                <input  class="form-control" name="end_time" id="event-end-time" type="text" value="{{old('end_time')}}"><ul class="parsley-errors-list">{{ $errors->event->first('end_time') }}</ul>



                                <label for="fullname">Website* :</label>
                                <input  name="website" class="form-control" type="text" value="{{ old('website') }}"><ul class="parsley-errors-list">{{ $errors->event->first('website') }}</ul>
                        ...
                        <div class="col-md-6 col-xs-12">
                            <div class="x_content">
                                ...
                                <select style="width:100%;" class="form-control" name="organizers" id='organization' data-placeholder="Choose a Organization...">
                                    ...
                                    <option <?php echo (old('organizers') == $organization_val->organization_id) ? "selected" : ''; ?> value='{{$organization_val->organization_id}}'>{{$organization_val->organization_name}}</option>
                                    @endforeach;   
                                ...
                                <select class="form-control select_input" name="location" id="event-location" style="width:90%">
                                    ...
                                    <option <?php echo (old('location') == $location_val->venue_id) ? "selected" : ''; ?> value='{{$location_val->venue_id}}'>{{$location_val->venue_name}}</option>
                                    @endforeach;   
                                </select>
                                ...
                                <select name="audience[]" id="audience" class="form-control select2-multiple" multiple>
                                    <optgroup>
                                        <option <?php
                                        if (!empty(old('audience'))) {
                                            if (in_array('Child', old('audience'))) {
                                                echo "selected";
                                            }
                                        };
                                        ...
                                        if (!empty(old('audience'))) {
                                            if (in_array('Youngest', old('audience'))) {
                                                echo "selected";
                                            }
                                        };
                                        ?>>Youngest</option>
                                        ...
                                        if (!empty(old('audience'))) {
                                            if (in_array('Oldest', old('audience'))) {
                                                echo "selected";
                                            }
                                        };
                                        ?>>Oldest</option>
                                ...
                                <input id="event_contact" name="contact_info" data-inputmask="'mask' : '(999) 999-9999'" id="contactinfo" value="{{ old('contact_info') }}" class="form-control textfield" type="tel">
                                <ul class="parsley-errors-list">{{ $errors->event->first('contact_info') }}</ul>



                                <label for="email">Event Purpose :</label>
                                <input name="purpose" id="event-purpose"  value="{{ old('purpose') }}" class="form-control textfield" type="text">
                                ...
                                <select name="keyword[]" id="category" class="form-control select2-multiple" multiple>
                                    ...
                                    <option <?php
                                    if (!empty(old('keyword'))) {
                                        if (in_array($val->category_name, old('keyword'))) {
                                            echo "selected";
                                        }
                                    };
                        ...
                        <div class="col-md-12 col-xs-12">
                            <label class="control-label">Event Description *</label></br>
                            <textarea class="ckeditor form-control" id="event_description" name="event_description">{{ old('event_description') }}</textarea>
                            <ul class="parsley-errors-list">{{ $errors->event->first('event_description') }}</ul>
                        </div>

                        <div class="clearfix"></div></br></br>
                        <div class="col-md-12 col-xs-12">
                            <label class="control-label">Select Featured Image</label>
                        ...
                        <div class="form-group">
                            <div class="col-md-6 col-sm-6 col-xs-12 col-md-offset-10  pull-left">
                                <button type="button" id="cancel" data-href="{{URL::to('event')}}" class="btn btn-primary">Cancel</button>
...
//                        }
//                        }, complete:function(){
//                var old_organization = ('{{old('organizers')}}');
//                        if (old_organization != ''){
//                $(organization).val(old_organization);
//                        $(organization).trigger("chosen:updated");
//                }
//
//                }
